...
- 하지만 이상의 방식은 예제에서 볼 수 있듯이 코드가 길어지고, 불필요한 반복이 많아지며,
    가독성이 떨어지는 문제가 있었습니다.(call1() 유형의 코드를 5개 정의한다고 하면 
...
3. 구현부/실행부(한국어판 intellij에서는 메서드 본문)
...
call3()만 필요한데 나머지도 정의해야한다는 문제점이 발생하기 때문에 각 유형 당 하나의 인터페이스/추상메서드
만 존재하게 됐습니다.
...
5. Predicate - boolean test(T t); - 얘는 return 타입이 boolean으로 고정이라 call1~call4로
    묶기 애매하긴 한데 굳이 따지자면 call4();입니다. return true / false 중 하나니까요.
...
import java.utio.function.Predicate;
...
ctrl + shift + f 전체에서 검색가능
ex) 특정 메서드 이름, 특정 자료형 , 키워드를 적으시오 etc..
