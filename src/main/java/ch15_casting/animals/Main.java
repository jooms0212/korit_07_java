...
public class Main {
    public static void main(String[] args) {
        ...
        // : 업캐스팅이 이루어지지 않고 그냥 Dog dog1 = enw Dog();로 만든 객체 역시
        ...
        System.out.println("↑ 업캐스팅 파트 - 객체명.메서드명(); 했을 때 재정의 버전이 나오는거 아닌가에 주목");
...
//        dog3.makeSound(); // 그러니까 얘는 지금 호출도 못해보는 상황입니다.
        /*
            69번 라인에서 오류가 발생했습니다.
            즉 애초에 animal3을 생성할 때 Animal 클래스의 생성자를 호출해서 만들었기 때문에
            (A a = new A(); 이기 때문에)
        ...
        if (animal4 instanceof Dog) {       // 이 조건식이 true라면 animal4의 생성시 호출된 생성자는 Dog();
            Dog dog4 = (Dog) animal4;       // 그러므로 87번 라인의 실행문을 통해 다운캐스팅을 하더라도 오류 발생 x
