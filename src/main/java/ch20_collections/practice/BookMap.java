...
/*
    ...
    3. 사용자로부터 수량을 변경할 도서명과 새로운 재고 수량을 입력 받아 Map의 값을 '수정'
        - 존재하지 않은 도서라면 "해당 도서가 재고에 없습니다." 출력
...
public class BookMap {
    ...
    while (!endOfProgram) {
        ...
        if (option == 1) {
            ...
            if (bookInventory.containsKey(bookName = scanner.nextLine())) {
                System.out.println("새로운 재고 수량을 입력하세요 >>>");
                stock = scanner.nextInt();
                scanner.nextLine();
                bookInventory.replace(bookName, stock);
                System.out.println(bookName + "도서가 " + stock + "권으로 변경 되었습니다.");
            } else {
