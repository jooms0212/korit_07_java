...
/*
    ...
    3. 메서드
        ...
        2) deposit(int amount, int inputPin) -> 입금 관련 메서드 / call2() 유형으로 작성
            ...
            - 콘솔창에
                0000원이 입금되었습니다 입금 성공 ! 현재 잔액 : 00000원

            withdraw(int amount, int inputPin) -> 출금 관련 메서드 / call2() 유형으로 작성
                - 올바른 비밀번호인지 확인 -> 틀리면 "비밀번호가 틀렸습니다."하고 메서드 종료
                - 유효한 금액인지 검증(amount 0초과) -> 실패하면 "불가능한 입력 금액입니다."
                - 출금 후 잔액이 0이상인지 확인 -> 음수라면 "출금이 불가입니다."
                - 콘솔창에
                    0000원이 출금되었습니다 출금 성공 ! 현재 잔액 : 0000원
        ...
        잔액이 부족하여 출금할 수 없습니다.                    # account1에서 200000원 출금 실패 사례(이십만원)
...
public class Bank {
    ...
    public void setPinNumber(int pinNumber) {
        if (pinNumber > -1 && pinNumber < 10000) {
            this.pinNumber = pinNumber;
        } else
    ...
    public void deposit(int amount, int inputPin) {
        if (inputPin != pinNumber) {
            ...
            System.out.println("입금성공! 현재 잔액 : " + balance);
        } else
    ...
    public void withdraw(int amount, int inputPin) {
        if (inputPin != pinNumber) {
            ...
            System.out.println("출금성공! 현재 잔액 : " + balance);
        } else System.out.println("잔액이 부족하여 출금할 수 없습니다");
    }
